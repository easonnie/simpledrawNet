...
class Scence2Image(nn.Module):
    def __init__(self, encoding_d=9, obj_vec_d=100):
        ...
        # self.fc_dcgan = FC_DCGAN(obj_vec_d * 8, mlp_d=4096, nc=3, ngf=128)
...
class Obj2Vec(nn.Module):
    def __init__(self, encoding_d=9, mlp_d=1024, obj_vec_d=100):
...
class FC_DCGAN(nn.Module):
    def __init__(self, obj_vec_d=100, mlp_d=1024, nc=3, ngf=128):
        super(FC_DCGAN, self).__init__()

        self.fcs = nn.Sequential(
            # input is Z, going into a convolution
            # state size. nz x 1 x 1
            nn.Linear(obj_vec_d, mlp_d),
            )

...
def unpack_from_linear_shuffle(inputs, lengths, pad_l=8):
    batch_list = []
    lengths = list(lengths.data)
    ...
    for l in lengths:
        ...
        pad_seq = pad_1d(inputs[start:end], pad_l=pad_l)
        shuffled_pad_seq = shuffle_seq(pad_seq)
        batch_list.append(shuffled_pad_seq.view(-1))
        start = end
    return torch.stack(batch_list, dim=0)
...
class Scence2Image(nn.Module):
    def __init__(self, encoding_d=9, obj_vec_d=100):
        super(Scence2Image, self).__init__()
        self.obj2vec = Obj2Vec(encoding_d, mlp_d=1024, obj_vec_d=obj_vec_d)
        self.fc_dcgan = FC_DCGAN(obj_vec_d * 8, nc=3, ngf=128)
    ...
    def forward(self, ss, ls, max_l=8):
        ...
        obj_vecs = unpack_from_linear_shuffle(obj_vecs, ls)

        image_outs = self.fc_dcgan(obj_vecs) # [B, T, C, H, W]
        return image_outs

    def forward_none_shuffle(self, ss, ls, max_l=8):
        # ss : [B, T, D]
        # ls : [B]
        packed_ins = pack_sequence_for_linear(ss, ls)
        obj_vecs = self.obj2vec(packed_ins)
        obj_vecs = unpack_from_linear(obj_vecs, ls)
...
class FC_DCGAN(nn.Module):
    ...
    def forward(self, obj_encoding):
        in_vec = obj_encoding
        in_vec = in_vec.view(-1, self.obj_vec_d, 1, 1)
...
def produce_dev_images():
    ...
    Image.fromarray(util.tensor2im(g_im)).save("vector_shifting_3.png")
